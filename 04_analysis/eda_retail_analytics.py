...
def fig_delivery_vs_satisfaction(con):
    ...
    # Si es modo mock o demo, relajamos el filtro de HAVING
    min_orders = 100 if os.environ.get("MOCK_DATA") != "true" else 1
    ...
    df = con.execute(f"""
        ...
        GROUP BY dc.customer_state
        HAVING ordenes >= {min_orders}
    """).fetchdf()

    if df.empty:
        print("  [!] Saltando Figura 4: No hay suficientes datos para el scatter plot.")
        return

    fig, ax = plt.subplots(figsize=(12, 8))
    # Ajuste de tamaño de puntos para que se vean incluso con pocas ordenes
    point_size = df['ordenes'] / 10 if os.environ.get("MOCK_DATA") != "true" else df['ordenes'] * 50
    ...
    # ... (rest of the labeling code)

    # Linea de tendencia (solo si hay al menos 2 puntos con X diferentes)
    if len(df) >= 2 and df['dias_entrega'].nunique() >= 2:
